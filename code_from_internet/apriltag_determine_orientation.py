...
def main():
    ...
    while True:
        ...
        for tag in tags:
            ...
            draw_cube(frame, np.array(tag.corners, dtype='float32'), tag_size, scale_factor, tag.tag_id)
...
def draw_cube(frame, corners, tag_size, scale_factor, tag_id):
    ...
    # Adjust the cube corners based on the tag ID
    if tag_id == 0:  # Front
        z_offset = 0
    elif tag_id == 1:  # Back
        z_offset = -cube_size
        ...
        z_offset = -half_size
    elif tag_id == 4:  # Bottom
        z_offset = half_size
    else:
        z_offset = 0  # Default case, can be adjusted as needed

    cube_corners_3d = np.float32([
        [-half_size, -half_size, z_offset], [half_size, -half_size, z_offset], 
        [half_size, half_size, z_offset], [-half_size, half_size, z_offset],
        [-half_size, -half_size, z_offset - cube_size], [half_size, -half_size, z_offset - cube_size], 
        [half_size, half_size, z_offset - cube_size], [-half_size, half_size, z_offset - cube_size]
    ])
